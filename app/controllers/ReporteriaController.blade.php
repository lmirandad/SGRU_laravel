	...
	public function generar_reporte_requerimientos()
	{
		if(Auth::check()){
			...
			if($data["user"]->idrol == 1 || $data["user"]->idrol == 2){
				...
				$value = Excel::create('Reporte Logs ', function($excel) use  ($fecha_desde_requerimiento,$fecha_hasta_requerimiento){
					$excel->sheet('Reporte', function($sheet) use ($fecha_desde_requerimiento,$fecha_hasta_requerimiento)  {
						$sheet->row(1, array(
							     'N°','CODIGO_SOLICITUD','FECHA_SOLICITUD','FECHA_ASIGNACION','FECHA_INICIO_PROCESANDO','FECHA_CIERRE','TIPO_SOLICITUD','ESTADO_SOLICITUD','HERRAMIENTA_SOLICITADA','APLICATIVO_AGRUPADO','ENTIDAD','CANAL','SECTOR','USUARIO_ASIGNADO','ASUNTO','SLA_PENDIENTE','SLA_PROCESANDO'
							));
						$solicitudes = Solicitud::buscarSolicitudesPorFechas($fecha_desde_requerimiento,$fecha_hasta_requerimiento)->get();
